...
# Author: Kane Zhan
# Purpose: To allow for functional verification of the N-bit Ripple Carry Adder (RCAN) topology. 
# Goals:
...
# set usage "Version: v$version \n\n\n Usage: do RCANfv.do \[options]" 

# 1. Name a unique transcript file
# ../Documentation/OutputFiles/RCAN_Testing_Transcript_{yyyy-mm-dd-hh-mm-ss}
# Time 
set systemTime [clock seconds]; list
set timestamp [clock format $systemTime -format "%Y-%m-%d_%H-%M-%S"]; list

set transcript_file "../Documentation/OutputFiles/RCAN_Testing_Transcript_$timestamp.txt"; list
# 2. Turn on/off re-direction to this transcript file when you wish to capture the output
...
if {$capture_set == 1} {
    # 3. Compile all relevant sourcecode  
    vcom -2008 ../SourceCode/Mux2c1b.vhd
    ...
    # 5. Setup an appropriate wave window
    # 6. Run the simulation
    vsim -c TBRCAN -l $transcript_file -do {
    do waveRCAN.do  
    run -all      
    }
} else {
